...
def spearman_corrcoef(predictions, targets) -> dict:
    ...
    if math.isnan(spearman_corrcoef):
        spearman_corrcoef = 0
    return {"spearman": spearman_corrcoef}
...
def build_compute_metrics_fn(task_names, tokenizer, ignore_pad_token_for_loss):
    ...
    def compute_metrics(eval_preds, eval_metrics, post_processor=None):
        ...
        for metric in eval_metrics:
            ...
            if metric=='f1':
                metric = 'f1_score_with_invalid'
            elif metric=='f1_multiclass':
                metric = 'mean_multiclass_f1(num_classes=3)'
            elif metric== 'pearson' or metric== 'spearman':
                metric += '_corrcoef'
            elif metric== 'matthews_correlation':
                metric='matthews_corrcoef'
            # multirc独立设定
            result.update(globals()[metric](decoded_preds, decoded_labels))
        return result
    ...
    def tasks_metrics(task):
        ...
        return functools.partial(compute_metrics, eval_metrics=TASK_TO_METRICS[task], post_processor=post_processor) # , metrics=TASK_TO_METRICS[task]
